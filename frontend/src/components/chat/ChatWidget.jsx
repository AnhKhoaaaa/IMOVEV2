  ...
  if (!user) {
    return open ? (
      ...
      <div className="fixed bottom-5 right-5 z-[100] flex flex-col items-center">
        {isHome && showBubble && (
          <div className="relative mb-4 w-52 rounded-2xl bg-white p-3 text-xs text-slate-800 shadow-xl border border-slate-100 scale-80 origin-bottom">
        ...
        {isHome ? (
          <button
            ...
            className="relative transition-transform hover:scale-105 active:scale-95 before:absolute before:-inset-8 before:z-0 before:animate-[pulse_3s_ease-in-out_infinite] before:rounded-full before:bg-[radial-gradient(circle,rgba(37,99,235,0.4)_0%,rgba(37,99,235,0)_70%)]"
          >
            <img src={merlionMascot} alt="Mascot" className="w-[225px] h-auto drop-shadow-xl relative z-10" />
          ...
          <button
            onClick={() => { setOpen(true); }}
            aria-label={t('chatOpen')}
            className="relative grid h-16 w-16 place-items-center rounded-full bg-white shadow-pop ring-1 ring-slate-200 transition-transform hover:scale-105 active:scale-95"
          >
            <img src={merlionAvatar} alt="Mascot" className="h-12 w-12 rounded-full object-cover" />
      ...
      <div className="fixed bottom-5 right-5 z-[100] flex flex-col items-center">
        ...
        {isHome ? (
          ...
          >
            ...
            <img src={merlionAvatar} alt="Mascot" className="relative h-12 w-12 rounded-full object-cover" />
            {unread > 0 && (
              <span
                aria-label={t('chatUnread', unread)}
