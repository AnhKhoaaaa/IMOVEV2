...
// dev25 P2 — the full interactive alert resolver (weather swap + closing-risk leave/skip/push
// + transport adapt/feedback) extracted from AlertBanner so it can be reused INSIDE the chat
// stream. `AlertBanner` is now a thin wrapper around this card; behaviour is identical, so the
// existing AlertBanner tests pin both. See docs/plans/dev25.md §Phase 2.
...
// dev20: running-late / closing-time card. Shows only the feasible resolutions and states
// the reason for any it can't offer — never a silent hide.
function ClosingRiskCard({ alert, tripId, onDismiss, onAdapted }) {
  const { t } = useT()
...
function GenericAlertCard({ alert, tripId, onDismiss, onAdapted }) {
  const { t } = useT()
...
export default function AlertActionCard({ alert, tripId, onDismiss, onAdapted }) {
  ...
  if (alert.alert_type === 'closing_risk') {
    return <ClosingRiskCard alert={alert} tripId={tripId} onDismiss={onDismiss} onAdapted={onAdapted} />
  }

  return <GenericAlertCard alert={alert} tripId={tripId} onDismiss={onDismiss} onAdapted={onAdapted} />
}
