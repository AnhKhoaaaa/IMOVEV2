...
**Critical:**
- [ ] **[P4-BUG-1] `_distribute_days` dùng fallback 15 phút cho pair thiếu trong `route_durations`** — `planning_agent.py:135`. Route pre-fetch chỉ cover consecutive pairs sau sort; khi best-effort fallback di chuyển place sang ngày khác, pair mới không có trong cache → `_distribute_days` giả sử 15 phút → clock simulation không chính xác, có thể schedule trip vượt 17:00 mà không bị detect.
- [ ] **[P4-BUG-2] Opening-hours check không tính dwell time** — `planning_agent.py:142`. Điều kiện hiện tại: `oh_open <= arrival <= oh_close`. Đúng phải là `arrival + dwell <= oh_close`. Một place với `opening_hours="09:00-10:00"` và `dwell=90min` vẫn được xếp nếu arrival=09:30 (visit kết thúc lúc 11:00, sau khi close).
- [ ] **[P4-BUG-3] Off-by-one trong best-effort fallback** — `planning_agent.py:153-161`. `days[best_day].append(place)` được gọi trước, sau đó check `len(days[best_day]) > 1` để tìm prev. Index `[-2]` bây giờ trỏ vào place trước place vừa append — đúng kết quả nhưng logic fragile, dễ break nếu refactor.

**Important:**
- [ ] **[P4-BUG-4] `_RATE_LIMIT_LOCK` giữ lock qua `asyncio.sleep`** — `gemini.py:41-45`. Sleep nằm bên trong lock → concurrent callers block nhau tối đa 4 giây, stacking delay dưới tải cao.
- [ ] **[P4-BUG-5] `END_MIN=1020` (17:00) vs `1050` (17:30) trong `_check_schedule_fit`** — 30 phút leeway không được document, boundary zone 17:01–17:29 không có test.
- [ ] **[P4-BUG-6] `pendingSave` bị mất khi user refresh trang** — `Trip.jsx:80-83`. `navState` là session-only → refresh trước khi save khiến trip orphaned (tồn tại backend, không có trong saved list, banner biến mất). Fix đề xuất: persist `pendingSave` vào `sessionStorage` keyed by trip ID.
- [ ] **[P4-BUG-7] Save banner invisible khi arrive từ Planner** — `Trip.jsx:83`. Tab mặc định là `d1` khi `pendingSave` set; banner nằm ở Summary tab → user phải tự tìm. Cần UI nudge "Review days → Save in Summary".
